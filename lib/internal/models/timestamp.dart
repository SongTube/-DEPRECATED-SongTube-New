      ...
      if (item.contains(':')) {
        ...
        if (item.contains('\n')) {
          ...
          if (newLineItems.length == 2) {
            ...
            try {
              final duration = _parseDuration(firstItem.replaceAll('\n', ''));
            ...
            try {
              final duration = _parseDuration(secondItem.replaceAll('\n', ''));
          ...
          // Case where we have a clusterfuck of text separated with new lines only
            ...
            for (var newLineItem in newLineItems) {
              try {
                final duration = _parseDuration(newLineItem.replaceAll('\n', ''));
          ...
          try {
            final duration = _parseDuration(item.replaceAll('\n', ''));
